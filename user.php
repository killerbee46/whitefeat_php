...
<head>
 ...
 <title><?php if($merchant==1){echo'Vendor';}else{echo'Customer';}?> Panel</title>
 <meta name="viewport" content="width=device-width, initial-scale=1">
...
<div class="columns" style="margin:1.5em; background: rgb(241,243,244);
  ...
  <div class="column has-text-left is-3 has-background-light each_item" style="border-radius:0;">
	<div class="user-detail">
		<div class="user-detail-avatar-wrapper">
			...
			<i class="fas fa-user-circle is-size-2" style="font-size: 100px !important; margin-top:20px;"></i>
			<button title="Update Profile" data-target="modal-uset" class="user_setting_container user_setting">
</button>
</div>
<span>Since</span>
...
<span class="tag is-info is-light is-size-6 letter-spacing">
			<?php if(!empty($rowus['email'])){echo $rowus['email'];}else{echo "Email N/A";} ?> </span>
			</span>
			<span class="tag is-info is-light is-size-6 letter-spacing">
			<?php if(!empty($rowus['address'])){echo $rowus['address'];}else{echo "Address N/A";} ?> </span>
			</span>
		</div>
	</div>
      
	  <div class="user-action-container">
	    <button class="button is-danger is-outlined sign-out">Logout &nbsp; <i class="fas fa-sign-out-alt"></i></button>
		<button class="button is-success is-outlined user_setting" data-target="modal-uset2" >Write a review &nbsp; <i class="fas fa-edit"></i></button>
	  </div>
  ...
  <div class="column modal-content has-background-white p-5 has-text-left" style="width:80VW; max-height:90VH!important;">
      
	  
	  <h3 class="subtitle is-size-5 ">Manage your profile:</h3><hr>
	  ...
	  <div class="column is-8 " style="margin-top:0em;">
	    Name: <br> <input class="input is-halfwidth mt-1 user-name" type="text" value="<?php  echo ucfirst($rowus['name']); ?>"></input>
	  </div>
   	  
	  <div class="column is-8 " style="margin-top:0em;">
	    Phone no.: <br> <input type="text" class="input is-halfwidth mt-1 user-phone" value="<?php  echo ucfirst($rowus['phone']); ?>"></input>
	  </div>
	  
	  <div class="column is-8 " style="margin-top:0em;">
	    Email: <br> <input type="text" class="input is-halfwidth mt-1 user-email" value="<?php  echo ucfirst($rowus['email']); ?>"></input>
	  </div>
	  <div class="column is-8 " style="margin-top:0em;">
	    Address: <br> <input type="text" class="input is-halfwidth mt-1 user-address" value="<?php  echo ucfirst($rowus['address']); ?>"></input>
	  </div>
	  
	  <div class="column is-8 " style="margin-top:0em;">
	    <button class="button is-info is-light save_details"><i class="fas fa-save"></i> &nbsp;Save details</button>
	  </div>
	    ...
	    New Password: <br> <input type="password" class="input is-halfwidth mt-1 np1" value=""></input>
	  </div>
	  
		 <div class="column is-6 " style="margin-top:0em;">
	    Confirm New Password: <br> <input type="password" class="input is-halfwidth mt-1 np2" value=""></input>
	  </div>
	  
		 <div class="column is-6 " style="margin-top:0em;">
	     <button class="button is-warning is-light save_pass"><i class="fas fa-save"></i> &nbsp;Save password</button>
	  </div>
...
background: linear-gradient(90deg, rgba(241,243,244,1) 0%, rgba(226,225,219,1) 34%, rgba(80,225,255,1) 80%, rgba(116,228,250,1) 98%);">Complete your profile:</h3><hr>
  ...
  <div class="modal-background"></div>
  <div class="column modal-content has-background-white has-text-left" style="width:50VW; max-height:95VH!important; overflow-x:hidden; ">
      
	  
	  <h3 class="subtitle is-size-5 p-4" style="background: rgb(241,243,244);
background: linear-gradient(90deg, rgba(241,243,244,1) 0%, rgba(226,225,219,1) 34%, rgba(80,225,255,1) 80%, rgba(116,228,250,1) 98%);
margin-top:-0.6em; margin-left:-1.5%; width:105.5%;">
Share your amazing experience&nbsp; <img src="assets/images/extra/happy.png" style="position:absolute; height:1em; margin-top:0.28em; margin-left:0.2em;">
</h3>
	  ...
	  <div class="column is-8 " style="margin-top:0em;">
	    Name: <small>*</small><br> 
		<input class="input is-halfwidth mt-1 user-name" name="customer_name" type="text" value="<?php  echo ucfirst($rowus['name']); ?>"></input>
	  </div>
	  ...
	  <div class="column is-12 " style="margin-top:0em;">
		...
		<textarea class="input is-fullwidth" name="message" style="height:15VH;" id="rv_msg_main"></textarea>

	  </div>
	  
	  <div class="column is-8 " style="margin-top:0em;">
	    <input type="submit" value="Submit for review" class="button is-success is-light"></input>
	  </div>
  ...
  <div class="column is-9 scroll-filter-product cart_items_div p-5 each_item" style="height:100vh;overflow-y:scroll; border:1px solid #ddd; border-left:0;  border-radius:0; background:whitesmoke;">
	<h3 class="subtitle">Purchase History:<small>( <span> <?php  
	...
	<?php 
      ...
      $displayup=mysqli_query($con,$sqlup); $sn=1; $total_net=0; $countxx=mysqli_num_rows($displayup);
	  if($countxx<=0){echo"<hr> You haven't made any purchases yet.... ";}
      ...
      if($rowup['c_request']=='1'){echo' disabled-del';}
	  echo'"  style="margin-top:0em;"> '; if($rowup['c_request']=='1'){echo'<span class="tag is-danger"> Processed for deletion</span> &nbsp; ';}
...
background: linear-gradient(90deg, rgba(241,243,244,1) 0%, rgba(226,225,219,1) 34%, rgba(80,225,255,1) 80%, rgba(116,228,250,1) 98%);">Tracker ID: <small><code>'.$rowup['tracking_code'].'</small></code>&nbsp; <small> #WFO-'.$rowup['cb_id'].' </small><small><small><small><small></small></small></small></small></h3>
      ...
      <li class="steps-segment 1step ';
       if($rowup['dispatch']==0){echo'is-active has-gaps';}
	  echo'">
	  ...
	  if($rowup['dispatch']==1 && $rowup['deliver']==0 ){echo'is-active has-gaps';}
	  if($rowup['dispatch']==1 && $rowup['deliver']==1 ){echo'has-gaps';}
	  if($rowup['dispatch']==0)
		  {echo'has-gaps';}
...
if($rowup['deliver']==1){echo'is-active';}
	  else{
	  echo'has-gaps';}
	   echo'">
...
<?php if($merchant==1){echo"$('#vendor_on').html('Vendor');";} ?>
</script>
</body>
...
var dataString = 'name='+$('.user-name-new').val()+'&address='+$('.user-address-new').val()+'&pass='+$('.np2x').val()+'&email='+$('.user-email-new').val();
   $.ajax({url: "saveudetail_new.php",data: dataString, type: 'POST', cache: false,
