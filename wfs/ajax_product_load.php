...
$query_s = "Select * from `whitefeat_wf_new`.`package` where cat_id='".$_GET['catid']."' order by p_name"; 
                    $display_s = mysqli_query($con,$query_s);  $sn=1;		 
                     ...
                     while($row_s2 = mysqli_fetch_array($display_s2))
			          {
							  ...
							  echo '<img src="../assets/images/product/thumb/'.$row_s2['thumb'].'" class="col-6 mb-1" style="height:50px;"/>';
					  }
							  ...
							  <div class="col-4">';
							   ...
							   echo '<i class="fab fa-youtube" style="font-size:3.5em; color:red; cursor:pointer;"';?> onclick="window.open('https://www.youtube.com/watch?v=<?php echo $row_s['p_vpath']; ?>', '_blank', 'location=yes,height=570,width=520,scrollbars=yes,status=yes');" > <?php echo'</i>';
							   }
							   else{echo'<small><small class="lspace"> NOT AVAILABLE<i class="fas fa-video" style="opacity:0.4; font-size:1.5em;"></i> </small></small>';}
							  echo'</div>
...
if(($sn%4)==0){echo'<div class="w-100"></div>';}						
