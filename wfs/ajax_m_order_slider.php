	...
	$('.columns').sortable({
		...
		stop: function(event, ui){
			...
			var dataString = 'data1='+sortorder;
             $.ajax({
	           type: "POST",
               url: 'ajax_m_order_save_slider.php',
               data: dataString,
	           cache: false,
...
<div class="mdis">
<h4 style="color:#fff;"> &nbsp;&nbsp;Drag & Drop Slider Order</h4>
	<div class="columns" style="padding:15px;">
	    <?php 
		include('db_connect.php');
		$sql = "Select * from `whitefeat_wf_new`.`slider` order by s_order ASC";
        $display = mysqli_query($con,$sql);		
        while($row = mysqli_fetch_array($display))
		{
	    echo'<div class="dragbox" id="'.$row['s_id'].'">
		<a href="#" style="cursor:move; color:#fff;"><h5 class="move_h5" >'.ucfirst($row['s_title']).'</h5></a>
		</div>';
		}
	   ?>
	</div>
