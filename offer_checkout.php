...
?>
    <div class="title flex align-center" style="margin-left:20px; gap:10px; margin-top:40px;">
      <i class="fas fa-shopping-cart"></i> Buy 12-12 Offer Nose Pin<span>
        (<?php

        $sql1act = "Select * from `whitefeat_wf_new`.`cart_book` where cookie_id='1454486' and checkout='0' ";
        ...
        $countcw = 0;
          ...
          $sql1acw = "Select * from `whitefeat_wf_new`.`cart_detail` where cart_id='293' ";
...
<div class="columns has-background-white mt-3 is-multiline each_item mr-3 delivery-info"  id="delivery-info"
                style="display:none;">
                ...
                <div class="column is-12 p-3 pl-5 " style="border-top:1px solid #eee; background:#eee;">
                  <span class="pl-0 is-size-6 has-text-weight-semibold is-fullwidth"><i
                      class="fas fa-check has-text-success is-size-5"></i> &nbsp; 1.&nbsp; Logged In: 
                      <?php 
                ...
                <div class="column is-6 p-5 has-background-white">
                  ...
                  <span id="">
                    ...
                    <form method="post" action="./ajax_create_order.php">
                      <div class="field mt-5">
                      <label class="label">Name:</label>
                      <p class="control has-icons-left">
                        <input name="name" class="input del-name" type="text" id="" value="<?= $rowud['name'] ?? "" ?>" placeholder="Contact Person Name">
                    ...
                    <div class="field">
                      ...
                      <p class="control has-icons-left">
                        <input name="phone" value="<?= $rowud['phone'] ?? "" ?>" class="input del-phone" type="text" id="exampleFormControlInputPass"
                    ...
                    <div class="field ">
                      ...
                      <p class="control has-icons-left has-icons-right">
                        <input name="address" value="<?= $rowud['address'] ?? "" ?>" class="input del-location" type="text" id="" placeholder="Nearest Landmark">
                        <span class="icon is-small is-left">
                    ...
                    <div class="field">
                      ...
                      <p class="control has-icons-left">
                        <textarea name="msg" class="input del-msg" type="text" id="exampleFormControlInputPass"
...
background: linear-gradient(90deg, rgba(241,243,244,1) 0%, rgba(226,225,219,1) 34%, rgba(80,225,255,1) 80%, rgba(116,228,250,1) 98%);">
                          Place Order ( COD ) &nbsp; <i class="fas fa-arrow-right"></i>
                        </button>
                      </p>
                    </div>
                    </form>
      ...
      <div class="container pt-5 checkout_div  " id="checkout_div">
        ...
        <h4 class="checkout_h4">Total (<?php echo $countcw; ?> Item<?php if ($countcw > 1) {
             ...
             if ($GLOBALS['customer'] != 0) {
               ...
               $sqlcrc = "Select cookie_currency from `whitefeat_wf_new`.`cookie_status` where cookie_id='1454486'";
               $displaycrc = mysqli_query($con, $sqlcrc);
               $rowcrc = mysqli_fetch_array($displaycrc);
               $sel_cur = $rowcrc['cookie_currency']??1;
             }
  ...
  overflow-y:scroll;">
            ...
            <span class="cart_area">
              ...
              while ($rowactx = mysqli_fetch_array($displayacw)) {
                $sqlckp = "select * from package where id_pack = ".$rowactx['id_pack'];
                // fetchProducts("id_pack='" . $rowactx['id_pack'] . "'");
	...
	<div class="column is-6 pb-0 has-text-right checkout_right_div">';
              ...
              <div class="columns pl-4 pr-4">
                ...
                <div
                  ...
                  <?php echo $cnot . " " . floor(($total_net+100/$crate));
                  echo '<input type="hidden" id="tcost" value="' . floor(($total_net+100/$crate)) . '" />'; ?>
                </div>
              </div>
            ...
            <div class="columns mt-4 p-3" style="cursor:<?= $offerExpired ? "not-allowed":'default' ?>" title="<?= $offerExpired ? "Offer Has Expired":'default' ?>">
              <?php 
              if ($GLOBALS['customer'] == '0') { ?>
                <button class="button is-normal is-fullwidth user-modal <?= $offerExpired ? "disabled" : "" ?>"
...
background: linear-gradient(90deg, rgba(241,243,244,1) 0%, rgba(226,225,219,1) 34%, rgba(80,225,255,1) 80%, rgba(116,228,250,1) 98%); border:0;"><i
              ...
              <?php } else { ?>
                <button class="button is-normal is-fullwidth <?= $offerExpired ? "disabled" : "" ?>" onclick="checkoutDetails()" style="background: rgb(241,243,244);
                <?= $offerExpired ? "disabled" : "" ?>
background: linear-gradient(90deg, rgba(241,243,244,1) 0%, rgba(226,225,219,1) 34%, rgba(80,225,255,1) 80%, rgba(116,228,250,1) 98%); border:0;"><i
                  class="fas fa-shopping-cart"></i> &nbsp; Checkout Now</button>
              <?php } ?>
            </div>
...
<form action="https://epay.esewa.com.np/api/epay/main/v2/form" method="POST" onsubmit="generateSignature()" id="esewa_pay">
    ...
    <script>
      ...
      $(document).on('click', '.proceed-payment', function () {
        var dataString = 'address=' + $('.del-location').val() + '&phone=' + $('.del-phone').val() + '&msg=' + $('.del-msg').val() + '&name=' + $('.del-name').val();

        $.ajax({
          url: "add_order", data: dataString, type: 'POST', cache: false,
...
function checkoutDetails() {
        const checkout_div = document.getElementById('checkout_div')
        const del_info = document.getElementById('delivery-info')
        checkout_div.style.display = "none"
        del_info.style.display = "block"
      }
</script>
