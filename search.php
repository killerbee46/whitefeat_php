...
<?php
    ...
    $title=''; $himage=0;  $hvideo=0;  $cterm=0; $cmenu=0; $ctag=0; 
...
foreach ($_GET['tags'] as $key => $tag) {
		if($tag=='kid'){$title=$title.' for Kids';}
	  if($tag=='men'){$title=$title.' for Men';}
	  if($tag=='women'){$title=$title.' for Women';}
	  if($tag=='gift'){$title=$title.'Gift';}
	  if($tag=='gift1'){$title=$title.'Birthday Gift';}
	  if($tag=='gift2'){$title=$title.'Anniversary Gift';}
	  if($tag=='gift3'){$title=$title.'Wedding Gift';}
	  if($tag=='gift4'){$title=$title.'Pasni Gift';}
			}
	  }
...
function queryFilter() {
	...
	if (array_key_exists('price',$_GET)) {
		...
		if (array_key_exists('tags',$_GET)) {
				 $tagv='';
	  foreach ($_GET['tags'] as $key => $tag) {
		if($tag=='kid'){$tagv='tag_kid > "0" ';}
	  if($tag=='men'){$tagv='tag_men > "0" ';}
	  if($tag=='women'){$tagv='tag_women > "0" ';}
	  if($tag=='gift'){$tagv='tag_gift > "0" ';}
	  if($tag=='gift1'){$tagv='tag_gift = "1" ';}
	  if($tag=='gift2'){$tagv='tag_gift = "2" ';}
	  if($tag=='gift3'){$tagv='tag_gift = "3" ';}
	  if($tag=='gift4'){$tagv='tag_gift = "4" ';}
...
<div class="" style="display:flex;justify-content:space-between; padding-top:30px; padding-bottom:10px;align-items:center;flex-wrap:wrap; gap:10px;">
   <div class="">
...
if (array_key_exists('sort',$_GET)) {
	 ...
	 echo' <i class="fas fa-tags"></i> <span id="count_design">('.$countslt2.'</span> Design'; if($countslt2>1){echo's';} echo')  </small>';
...
<div class="columns p-2 product-list-wrapper">
  ...
  echo'<a href="#" title="Add to wishlist" class="add_wish_owl" data-id="'.$rowslt2['id_pack'].'"><i class="far fa-heart is-size-4" style="color:#3892C6; cursor:pointer;"></i></a>';		  
	...
	<div class="column p-0">';
      ...
      $displaytg=mysqli_query($con,$sqltg); $countgw1=mysqli_num_rows($displaytg);
  ...
  echo'">'.strtoupper($rowtg2['tag_name']).'</span>
	...
	echo'</div>   
     ...
     if($GLOBALS['customer']!=0){
      ...
      $cnot=$rowcrc2['cur_name']; $crate=($rowcrc2['cur_rate']);
